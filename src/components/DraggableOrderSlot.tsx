...
interface DraggableOrderSlotProps {
...
export const DraggableOrderSlot: React.FC<DraggableOrderSlotProps> = ({
  scheduledOrder,
  date,
  ...
  const { selectedOrders, isMultiSelectMode, handleOrderSelect } = useDragDrop();
  ...
  const {
    ...
    transition,
    isDragging,
  } = useSortable({ id: scheduledOrder.id });
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    handleOrderSelect(scheduledOrder.id, e.ctrlKey || e.metaKey);
  };

  const handleCheckboxChange = (checked: boolean) => {
    handleOrderSelect(scheduledOrder.id, true);
  };
              ...
              <div className="flex-1 overflow-hidden">
                {!isHovered ? (
                  ...
                  <div className="space-y-1 text-xs">
                    ...
                    <div className="flex justify-between">
                      <span className="opacity-90">
                        <span className="font-medium">SMV:</span> {scheduledOrder.smv}
                      </span>
                      <span className="opacity-90">
                        <span className="font-medium">MO:</span> {scheduledOrder.moCount}
                      </span>
                    </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm font-medium">SMV:</label>
                <p>{scheduledOrder.smv}</p>
              </div>
              <div>
                <label className="text-sm font-medium">MO Count:</label>
                <p>{scheduledOrder.moCount}</p>
              </div>
            </div>
