...
export const AdvancedInventoryReport: React.FC = () => {
  ...
  // Helper to extract color-based category from product name
  const extractColorCategory = (productName: string): string => {
    if (!productName) return 'Unknown';
    // Remove size at the end first
    const sizePattern = /(\b(2XL|3XL|4XL|XL|L|M|S)\b|\b(22|24|26|28|30|32|34|36|38|40|42)\b)$/i;
    const nameWithoutSize = productName.replace(sizePattern, '').trim().replace(/[-\s]+$/, '');
    // Extract everything before the last space or dash as the color category
    const parts = nameWithoutSize.split(/[-\s]+/);
    if (parts.length >= 2) {
      return parts.slice(0, -1).join('-').toUpperCase();
    }
    return nameWithoutSize.toUpperCase();
  };

  // Generate color-based analysis
  const getColorBasedAnalysis = () => {
    const colorMap: { [colorCategory: string]: InventoryData[] } = {};
    
    inventoryData.forEach(product => {
      const colorCategory = extractColorCategory(product.product_name);
      if (!colorMap[colorCategory]) {
        colorMap[colorCategory] = [];
      }
      colorMap[colorCategory].push(product);
    });

    return Object.keys(colorMap).map(colorCategory => ({
      category: colorCategory,
      products: colorMap[colorCategory],
      totalStock: colorMap[colorCategory].reduce((sum, p) => sum + p.quantity_on_hand, 0),
      totalIncoming: colorMap[colorCategory].reduce((sum, p) => sum + (getPendingIncomingForProduct(p) || 0), 0),
      needsPlanning: colorMap[colorCategory].reduce((sum, product) => {
  ...
  const exportMainPlanningToExcel = () => {
    const data: any[] = [];
    ...
    categoryAnalysis.forEach(category => {
      ...
      data.push({
        ...
        'Product Category': '',
        'Sales Quantity': '',
        'Current Stock': category.totalStock,
        'Incoming': category.totalIncoming,
        'Stock + Incoming': category.totalStock + category.totalIncoming,
        'Needs Planning': category.needsPlanning > 0 ? category.needsPlanning : 'OK'
      });
      
      // Add products under category
      sortProductsBySize(category.products).forEach(product => {
        const salesQty = Math.round(getSalesQtyForProduct(product, parseInt(selectedMonths)) * (1 + salesQtyPercent / 100));
        const availableIncoming = getPendingIncomingForProduct(product);
        const stockWithIncoming = product.quantity_on_hand + availableIncoming;
        const needsPlanning = Math.max(0, salesQty - stockWithIncoming);
        ...
        data.push({
          ...
          'Incoming': availableIncoming,
          'Stock + Incoming': stockWithIncoming,
          'Needs Planning': needsPlanning > 0 ? needsPlanning : 'OK'
        });
      });
    });
    
    exportToExcel(data, `${selectedMonths}_Month_Planning_Analysis`, 'Planning Analysis');
  };

  const exportNextMonthToExcel = () => {
    const data: any[] = [];
    
    categoryAnalysis.forEach(category => {
      // Add category row
      const categoryUrgentQty = category.products.reduce((sum, product) => {
      ...
      data.push({
        ...
        'Incoming': category.totalIncoming,
        'Stock + Incoming': category.totalStock + category.totalIncoming,
        'Needs Urgent': categoryUrgentQty > 0 ? categoryUrgentQty : 'OK'
      });
      
      // Add urgent products only
      const urgentProducts = category.products.filter(product => {
        const salesQty = getSalesQtyForProduct(product, 1);
        return salesQty > 0 && product.quantity_on_hand < salesQty;
      });
      
      sortProductsBySize(urgentProducts).forEach(product => {
        const salesQty = getSalesQtyForProduct(product, 1);
        const availableIncoming = getPendingIncomingForProduct(product);
        const stockWithIncoming = product.quantity_on_hand + availableIncoming;
        const urgentQty = Math.max(0, salesQty - product.quantity_on_hand);
        ...
        data.push({
          ...
          'Incoming': availableIncoming,
          'Stock + Incoming': stockWithIncoming,
          'Needs Urgent': urgentQty
        });
      });
    });
    
    exportToExcel(data, 'Next_Month_Planning_Analysis', 'Next Month Analysis');
  };

  const exportColorBasedToExcel = () => {
    const colorAnalysis = getColorBasedAnalysis();
    const data: any[] = [];
    
    colorAnalysis.forEach(colorCategory => {
      // Add color category row
      data.push({
        'Product Name': `COLOR CATEGORY: ${colorCategory.category}`,
        'Product Category': '',
        'Sales Quantity': '',
        'Current Stock': colorCategory.totalStock,
        'Incoming': colorCategory.totalIncoming,
        'Stock + Incoming': colorCategory.totalStock + colorCategory.totalIncoming,
        'Needs Planning': colorCategory.needsPlanning > 0 ? colorCategory.needsPlanning : 'OK'
      });
      
      // Add products under color category
      sortProductsBySize(colorCategory.products).forEach(product => {
        const salesQty = Math.round(getSalesQtyForProduct(product, parseInt(selectedMonths)) * (1 + salesQtyPercent / 100));
        const availableIncoming = getPendingIncomingForProduct(product);
        const stockWithIncoming = product.quantity_on_hand + availableIncoming;
        const needsPlanning = Math.max(0, salesQty - stockWithIncoming);
    ...
    exportToExcel(data, `Color_Based_${selectedMonths}_Month_Analysis`, 'Color Analysis');
  };
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Button
              onClick={exportMainPlanningToExcel}
              variant="outline"
              size="sm"
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Button
              onClick={exportNextMonthToExcel}
              variant="outline"
              size="sm"
              className="flex items-center space-x-2"
            >
        ...
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span>Color-Based {selectedMonths} Month Planning Analysis</span>
            </div>
            <Button
              onClick={exportColorBasedToExcel}
              variant="outline"
              size="sm"
              className="flex items-center space-x-2"
            >
          ...
          <CardDescription>
            Product planning grouped by color variants (e.g., BRITNY-BLACK from BRITNY-BLACK M)
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-purple-700 text-white">
                  <th className="border p-2 text-left">Color Category</th>
                  <th className="border p-2 text-left">Product Category</th>
                  <th className="border p-2 text-right">Sales Quantity (Forecast)</th>
                  <th className="border p-2 text-right">Current Stock</th>
                  <th className="border p-2 text-right">Incoming</th>
                  <th className="border p-2 text-right">Stock + Incoming</th>
                  <th className="border p-2 text-right">Needs Planning</th>
              ...
              <tbody>
                {getColorBasedAnalysis()
                  .filter(colorCategory => colorCategory.products && colorCategory.products.length > 0)
                  .filter(colorCategory =>
                    (categorySearch.trim() === '' || colorCategory.category.toLowerCase().includes(categorySearch.trim().toLowerCase())) &&
                    !hiddenCategories.includes(colorCategory.category)
                  )
                  .map((colorCategory, index) => {
                    const isExpanded = categoryAnalysis.find(cat => cat.category === colorCategory.category)?.expanded || false;
                    return (
                      <React.Fragment key={colorCategory.category}>
                        <tr 
                          className="hover:bg-gray-50 cursor-pointer bg-purple-50"
                          onClick={() => {
                            // Toggle expansion for the matching category in categoryAnalysis
                            const matchingCategoryIndex = categoryAnalysis.findIndex(cat => cat.category === colorCategory.category);
                            if (matchingCategoryIndex >= 0) {
                              toggleCategoryExpansion(matchingCategoryIndex);
                            }
                          }}
                        >
                          <td className="border p-2 font-medium" colSpan={2}>
                            {isExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />} {colorCategory.category}
                          </td>
                          <td className="border p-2 text-right">-</td>
                          <td className="border p-2 text-right">{colorCategory.totalStock}</td>
                          <td className="border p-2 text-right">{colorCategory.totalIncoming}</td>
                          <td className="border p-2 text-right">{colorCategory.totalStock + colorCategory.totalIncoming}</td>
                          <td className="border p-2 text-right text-sm">
                            <span className={colorCategory.needsPlanning > 0 ? 'text-red-600 font-bold' : ''}>
                              {colorCategory.needsPlanning > 0 ? colorCategory.needsPlanning : 'OK'}
                            </span>
                          </td>
                          <td className="border p-2 text-center">
                            {!isExpanded && (
                              <Button
                                size="sm"
                                variant={colorCategory.needsPlanning > 0 ? "destructive" : "outline"}
                                onClick={e => {
                                  e.stopPropagation();
                                  setViewCategoryDialog({ open: true, category: colorCategory.category });
                                }}
                              >
                                View Products
                              </Button>
                        ...
                        {isExpanded && sortProductsBySize(colorCategory.products).map((product) => {
                          const salesQty = Math.round(getSalesQtyForProduct(product, parseInt(selectedMonths)) * (1 + salesQtyPercent / 100));
                          const availableIncoming = getPendingIncomingForProduct(product);
                          const stockWithIncoming = product.quantity_on_hand + availableIncoming;
                          const needsPlanning = Math.max(0, salesQty - stockWithIncoming);
